function fig = LG_6DoFAnimation( varargin )
...
ShowArrowHead = true;
ShowLegend = false;
Title = 'Quadcopter with a payload';
Position = [50 50 866 600];
Spin = 120;
% View = [(-20:(Spin/(length(pQ_total)-1)):(-20+Spin))', 25*ones(length(pQ_total), 1)];
View = [(-90:(Spin/(length(pQ_total)-1)):(-90+Spin))', 1*ones(length(pQ_total), 1)];
AxisLength = 1;
LimitRatio = 1;
Xlabel = 'X(m)';
Ylabel = 'Y(m)';
...
%%
    for i = 1:numPlotSamples
        ...
        xr4 = r0*cos(temp')*eb1 + r0*sin(temp')*eb2 + ones(size(temp'))*x4;
        ratio = 0.4;
        set(orgHandle, 'xdata', xQ, 'ydata', yQ, 'zdata', zQ,'Color','b','LineWidth',0.5);% 原点的句柄设置
        ...
        set(PorgHandle,'xdata', xP, 'ydata', yP, 'zdata', zP,'Color','g','LineWidth',0.1);
